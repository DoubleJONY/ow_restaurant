...
> 상태: 연습모드에서 에디션 데이터 세트를 다시 로드하는 구현과 검증은 별도 요청이 있을 때까지 보류한다. 아래 내용은 재개 시 참고할 설계 기록이며 현재 작업 범위가 아니다.
...
- 연습모드 에디션 재로딩은 현재 보류한다. 추후 재개하더라도 공용 손님 구성에는 에디션 분기를 추가하지 않는다.
...
- 최종 구조에서는 CAFE/GC의 튜토리얼 인덱스를 건너뛰는 분기를 두지 않는다. 현재의 `stage = 1` 강제·`stage 0` 건너뛰기·비ORG 튜토리얼 `Abort`는 튜토리얼 이식 시 제거할 임시 우회다.
- CAFE/GC 튜토리얼 콘텐츠와 에디션별 `STAGE_CODE` 데이터는 후속 작업으로 추가하되 공통 인덱스나 HUD 순서는 변경하지 않는다.
...
| 변수 | 비교 결과 | 처리 |
|---|---|---|
| `CUSTOMER_LIST` | CAFE=GC, ORG는 튜토리얼 항목 1개 추가 | ORG판을 공용화하고 CAFE/GC 튜토리얼도 같은 인덱스에 추후 추가 |
