...
> 상태: **완료 (2026-08-29)**. 최종 릴리즈 기준은 `release_v260829.md`를 따른다.
...
- 당시 Workshop import 실측은 32,746 / 32,768 elements였으며, 이후 직렬화 전 기준치로 보존한다.
...
- 단일 결과물 `kr_deluxe.ow`의 ORG/CAFE/GC 통합 작업을 완료 처리했다.
- 최종 버전은 `v260829`이며 데이터 생성기와 전체 조립기의 `--check`를 통과했다.
- CAFE/GC 튜토리얼 신규 작성, N3 4번째 에디션 통합, `STAGE_CODE` 추가 직렬화는 이번 릴리즈 범위에서 제외했다.
- 직렬화 후 실측은 `31,925 / 32,768`이며 당시 남은 예산은 843 elements였다. 직렬화 전 `32,746`에서 821 elements 감소한 값이며, 아래 `totalScore` 패치 직전 기준이다.
- 완료 산출물과 검증값은 `release_v260829.md`, 세부 검증은 `validation_report.md`에 기록했다.

## 에디션별 최고점 분기 보완 (2026-08-29)

- 초기 공용 구간에 있던 ORG 전용 `Global.totalScore` 대입을 제거했다.
- ORG/CAFE/GC 원본 최고점 배열을 에디션별 6행으로 보존하고 `Array Slice(..., Global.stageMode[0] * 6, 6)`로 선택한다.
- 대입 위치는 `selectMode` 반환 후, `Global.difficulty` 대입 직전으로 이동했다.
- 연습모드 에디션 전환에서는 최고점 표시를 사용하지 않으므로 `dataInit` 재호출 경로에는 넣지 않았다.
- 실측 31,925 / 32,768은 이 보완 직전 값이며, 패치 후 element 수는 재측정이 필요하다.
